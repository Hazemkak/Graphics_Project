...
our::Texture2D *our::texture_utils::loadImage(const std::string &filename, bool generate_mipmap)
{
    ...
    // make unpack alignment 1 byte 
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

    // specify a two-dimensional texture image
    // Parameters:  the target texture , nth Level of mipmap reduction image,number of color components,width, hight , the width of the border
    //              the format of the pixel data , the data type of the pixel data , pointer to the image data
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size.x, size.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, (void *)pixels);

    if (generate_mipmap)
    {
        // it genrates mipmap reduction images from the base image level to one pixel image.
