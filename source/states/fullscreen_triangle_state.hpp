...
class FullscreenTriangleState: public our::State {
    ...
    void onInitialize() override {
        ...
        /* here he already reads the center/radius/colors in case circle etc. */
        if(const auto& uniforms = config["uniforms"]; uniforms.is_object()){
            for(auto& [name, uniform] : uniforms.items()){
                ...
                if(type == "float"){ // circle's radius
                    float value = uniform.value("value", 0.0f);
                    program.set(name, value);
                } else if(type == "vec2"){ // for setting center W/L etc.
                    glm::vec2 value = uniform.value("value", glm::vec2(0,0));
                    program.set(name, value);
                } else if(type == "vec4"){ // for setting colors
        ...
        // generating the Vertex array and pass the defined vertexArray GLuint declared above
    ...
    void onDraw(double deltaTime) override {
        ...
        // We bind the vertexArray & then say to the program that we are going to draw Triangles
    ...
    void onDestroy() override {
        ...
        // Deleting the Vertex array on Destroying the program
